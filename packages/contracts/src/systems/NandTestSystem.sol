...
uint256 constant ID = uint256(keccak256("system.NandTest"));

contract NandTestSystem is System {
  ...
  function execute(bytes memory arguments) public returns (bytes memory) {
    ...
    require(false, "NandTest failed"); // this is really hacky but by failing the transaction, the user gets immediate feedback in the ui
    // the test failed
